...
11. **Efeitos Cosméticos e Refinamento Visual (Premium Layer):** Para garantir o acabamento sofisticado do Joinha DS, utilize as classes de utilidade customizadas (`.surface-highlight`, `.brand-glow`, `.elevation-1`, `.elevation-2`, `.border-gradient-subtle`) em componentes destacados (Cards principais, Modais, Botões Primários). No entanto, obedeça a regra estrita: **"Efeitos visuais (glows, inner highlights) nunca devem competir com a leitura de dados nem comprometer o contraste WCAG 2.2."** Nunca aplique `brand-glow` em botões secundários ou destrutivos, e evite sobreposição de gradientes que alterem o contraste do texto.
